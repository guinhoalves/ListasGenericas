...
class ItemListaPage extends StatefulWidget {
  ...
  ItemListaPage({this.idLista, this.nomeLista});
...
class _ItemListaPageState extends State<ItemListaPage> {
  ...
  List<ItemLista> itenslista = List();
  ...
  _getAllItensLista() {
    helper.getAllItensLista(widget.idLista).then((list) {
      if (list != null) {
        setState(() {
          itenslista = list;
        });
      ...
      child: Dismissible(
        ...
        child: CheckboxListTile(
          ...
          onChanged: (c) {
            setState(() {
              c == true
                  ? itenslista[index].status = 1
                  : itenslista[index].status = 0;
            });
            ...
            final snack = SnackBar(
              ...
              action: SnackBarAction(
                ...
                onPressed: () {
                  setState(() {
                    itenslista.insert(
                        _lastRemovedPos, ItemLista.fromMap(_lastRemoved));
  ...
  showDialogEditItem(BuildContext context, int index) {
    TextEditingController _updatedNomeItemLista =
        TextEditingController(text: itenslista[index].nome);
